...
TypeScript/UI field change from leaking into perception, planning, or risk
...
from ai_framework.common.timing import FrameReference
...
from ai_framework.runtime.model_deployment import ModelDeploymentResult
...
def plan_proposal_message(
    ...
    *,
    ...
    missing = [subtask.subtask_id for subtask in subtasks if subtask.subtask_id not in validations]
    if missing:
        raise ValueError(f"missing validation results for subtasks: {missing}")
    ...
    payload = {
        ...
        "subtasks": [
            {
                ...
                "preconditions": list(subtask.preconditions),
                "forbidden_if": list(subtask.forbidden_if),
                "required_evidence": list(subtask.required_evidence),
                "resource_requirements": dict(subtask.resource_requirements),
            }
        ...
        "validations": [
            {
                "subtask_id": subtask.subtask_id,
                "executable": validations[subtask.subtask_id].executable,
                "reasons": list(validations[subtask.subtask_id].reasons),
                "missing_evidence": list(validations[subtask.subtask_id].missing_evidence),
            }
            for subtask in subtasks
        ],
    ...
    return build_message(context, "plan_proposal", payload)
