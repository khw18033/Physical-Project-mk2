"""Scenario 5 — 말단↔엣지 네트워크 단절
Scenario 14 — 엣지 한 대 완전 장애 시 구역 격리

tests for: AI-N-01, AI-C-10, AI-C-11, AI-O-04, AI-B-06
question: ② 일부 기능이 없어져도 나머지가 계속 도는가?
"""

from ai_framework.contracts.capability import CapabilityRequirement, CapabilityState
from ai_framework.observability.availability import AvailabilitySignals, RemoteFeatureGate, SignalStatus
from ai_framework.providers.fakes import InMemoryTransportProvider
from ai_framework.reference.local_safety import LocalSafetyJudge, SafetyState
...
def test_s5_terminal_stops_answering_when_partitioned(registry):
    ...
    robot.start()
    transport.publish(robot.command_topic, b'{"command_id":"c1","command":"START_TASK"}')
    assert robot.command_log

    robot.go_offline()
    robot.command_log.clear()
    transport.publish(robot.command_topic, b'{"command_id":"c2","command":"START_TASK"}')
