"""Scenario 12 — 코드는 그대로 두고 Robot → River 전환
Scenario 13 — 개발이 끝난 뒤 감시(surveillance) 도메인을 새로 추가

tests for: AI-C-15, AI-C-04, AI-C-05, AI-B-09
question: ④ 로봇·감시·하천이 동일 프레임워크로 돌아가는가?
성공 조건: Core source code 변경 = 0 LOC.
"""
...
from ai_framework.contracts.profile_loader import is_capability_active, load_profile
from ai_framework.providers.fakes import InMemoryTransportProvider
from ai_framework.runtime.application import ZoneApplication
...
ALL_SPECS = [
    ...
    spec("decision.subtask_generate", rank=4),
    spec("decision.subtask_validate", rank=5),
...
def test_s12_robot_profile_activates_perception_and_subtasks_but_no_risk_fsm():
    profile, app = run_domain(PROFILE_DIR / "robot.json")

    assert app.state_of("perception.detect") is CapabilityState.ACTIVE
    assert app.state_of("decision.subtask_generate") is CapabilityState.ACTIVE
...
def test_s12_river_profile_activates_risk_and_leaves_subtasks_uninstalled():
    ...
    assert app.state_of("decision.subtask_generate") is CapabilityState.DISABLED
    # 사용하지 않는 기능은 설치·실행을 요구하지 않는다 (AI-C-15).
    assert "decision.subtask_generate" not in profile.active_capability_kinds
