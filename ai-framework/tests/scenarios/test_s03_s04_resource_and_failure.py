"""Scenario 3 — Edge CPU 포화 → 선택 기능 자동 축소·복구
Scenario 4 — 선택 Provider 강제 종료 → 기능 단위 격리

tests for: AI-B-06, AI-B-07, AI-C-05, AI-C-11, AI-C-13, AI-O-02
question: ② 일부 기능이 없어져도 나머지가 계속 도는가?
          ③ 실행 환경이 바뀌면 자동으로 재구성되는가?
"""

from ai_framework.contracts.capability import CapabilityRequirement, CapabilityState
from ai_framework.providers.fakes import InMemoryObservabilityProvider
...
def test_s4_killing_an_optional_provider_disables_only_dependent_capabilities(registry):
    ...
    profile = profile_of(
        "zone-1",
        ["perception.detect", "perception.track", "perception.depth", "decision.subtask_generate"],
    )
    specs = [
        spec("perception.detect", core=True),
    ...
    registry.register_local(registration("decision.subtask_generate", "planner-1"))
    ...
    assert states["decision.subtask_generate"] is CapabilityState.DISABLED
