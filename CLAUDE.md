...
- **채팅방 진입 규칙 (부트스트랩):** 새 설계방을 열 때는 설계방이 지켜야 할 규칙 문서
  `docs/be/설계_규칙.md`와 `docs/be/작업지시_템플릿.md`를 **반드시 함께 올린다.** 이 둘에
  "설계방이 지시서를 어떻게 만드는가"의 규칙(골격·서버 1단계·이월 흘리기·표로 결정·아침 메모
  형식)이 있어, 빠지면 새 방이 규칙을 모른 채 시작한다. 무엇을 올릴지 전체 목록은
  `docs/be/설계방_진입가이드.md` §1(고정 세트 11개 + Phase별 추가)에 있다 — 단 그 진입가이드
  자체는 **사람용 운영 절차**라 설계방에 올리지 않는다(AI가 읽으면 절차를 자기 행동으로 오해한다).
  **규칙이 문서에만 있고 방에 전달되지 않으면 없는 것과 같다.**
- **설계는 별도 채팅방에서** 하고, 그 결과를 작업 지시서로 굳혀 이 저장소에 넣는다. 작업
  지시서는 `docs/be/작업지시_템플릿.md`의 골격(대상·배경·제약·먼저읽을것·단계별DoD·이번에
  안하는것·완료판정·보고)을 따른다.
