...
- **채팅방 진입 규칙 (부트스트랩):** 새 설계방을 열 때는 `docs/be/설계방_진입가이드.md`와
  `docs/be/작업지시_템플릿.md`를 **반드시 함께 올린다.** 이 둘에 "설계방이 어떻게 일하는가"의
  규칙(올릴 파일 목록·지시서 골격·아침 txt·이월 챙기기)이 있어, 빠지면 새 방이 그 규칙을 모른
  채 시작한다. 무엇을 올릴지 전체 목록은 진입가이드 §1(고정 세트 11개 + Phase별 추가)에 있다.
