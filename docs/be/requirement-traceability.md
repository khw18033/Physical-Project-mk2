...
- **발표본과의 관계:** 캡스톤2 중간발표용으로 이 47건을 SRS 양식 28건으로 통합한 문서가 별도로
  ...
  기준은 이 문서(47건)이며 발표본은 표현 형식**이다. 발표본 ID ↔ 정본 ID 대응은 발표본 문서의
  부록 대응표에서 관리한다.
...
| DT-06 | 교차 구역 핸드오프·전역 트윈 | 미착수 | `backend/twin/` (Phase 7)<br>**계약:** 구역 내 track ID는 AI(AI-S-06)가, **전역 객체 ID는 백엔드**가 부여·매핑한다. `contracts/common/object-reference.schema.json` 참조 | — |
