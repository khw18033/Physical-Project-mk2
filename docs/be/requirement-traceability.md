...
- **총 47건 · 완료 3 · 부분 12 · 미착수 32** (2026-09-19, Phase 4 미디어 경로 단계 8 시점 — 4b 저장소는 단계 10)
- 완료 3건(BE-C-01·BE-T-02·**BE-S-01**)은 **동작과 허용 범위가 pytest로 보장**된 것이다
  (양성 + **음성 대조** + 실측). 각 행에 남은 범위 한계를 gap으로 적었다.
- 부분 12건: BE-T-01(LWT·인증·ACL 미구현) · BE-T-03(`/state` echo + `/media` 경로 분리·토큰·두 주소 바인딩) ·
  ...
  **BE-S-02**(파이프라인 3종·A층 14·C층 12 — 엣지 실물·인증·백엔드 span 없음) · **BE-S-03**(2계층을 임시 엣지로
  두 번 실증) · **BE-S-06**(경계 표기 규약 확정·적용) · **BE-S-07**(측정 수단만) · BE-C-02(레지스트리 관측 축까지) ·
  BE-S-05(감사 스키마까지) · BE-S-08(실행 기록 골격·append 경로까지).
- **Phase 4(미디어 경로)에서 바뀐 행:** **BE-T-07·BE-C-03·BE-T-08(미착수 → 부분)** · BE-T-03(경로 분리·토큰·외부
  뷰어 실측) · BE-T-02(원격 엣지 EDGE 리스너 실증 → 되돌림) · BE-S-02(라벨 8종·`LoggingHandler`·4316 재실증·브라우저
  입구 결정 7 A) · BE-S-03/S-06(페더레이션 2차 실증) · BE-T-05·BE-T-06·BE-S-09(gap·규약 갱신, 상태 유지)
...
| BE-S-09 | 미디어 저장 모드·이벤트 캡처 | 미착수(설계 확정 — 구현은 Phase 4 단계 10) | — (현재 배포는 중계만)<br>**유예 항목 — #15 갈래는 유예 해제(Phase 4 결정 9).** 스트림 저장(이벤트 캡처·상시 녹화)은 여전히 저장 배포 선택 시 설계(아키텍처 §8-5). 사건 이전 구간 확보를 위한 **엣지 단기 버퍼는 필요**하며 "상시 링버퍼 배제"로 읽히지 않도록 읽는다<br>**#15 촬영본 저장소 — 설계 확정(2026-09-18, `02-media-path.md` §1-3-4):** HW가 주행 중 촬영본(10fps JPEG 464×400, **0.7GB/h**)을 매니페스트+tar.gz로 묶어 단순 HTTP PUT(S3 API 아님 — `capture_upload.py:118-125`)하려고 목적지를 묻는 중. **ㄱ 파일시스템 + 작은 PUT 수신단**(`backend/gateway/capture.py`, 포트 8767, `<서버 tailscale IP>` 바인딩, `MK2_CAPTURE_TOKEN`, 별도 유닛 `mk2-capture`, 새 저장 제품 없음 — MongoDB 발동 조건 "프레임 낱장 증거+메타 동시 보관"은 파일시스템으로 충분해 미충족) · **PUT 두 번**(`<session>.manifest.json` 먼저 → 2xx → `<session>.tar.gz`; 아카이브 안을 뒤지지 않는다) · **최종 경로는 매니페스트의 `source_id`·`session_id`가 정한다**(URL 파일명이 아님 — 로봇 2대가 같은 분에 시작하면 충돌) · 경로 순회 방어는 매니페스트 값에 · 임시 파일→fsync→원자적 rename · MySQL `media_capture` 한 행 = 세션(매니페스트 원본 통째, `session_id` UNIQUE 멱등, `mk2_app`은 `SELECT, INSERT, UPDATE`) · **`frame_ref_base`는 채우지 않는다**(엣지를 거치지 않음, 시각 = `t0_unix + n×interval_s`) · 보존 = 용량 상한(`MK2_CAPTURE_MAX_BYTES` 100GB) + 오래된 세션부터 · **ㄴ 입구는 Phase 4 뒤에도 남긴다**(HW에 4b 완료 후 통지)<br>**gap:** 구현·검증(합성 업로더 C1~C5)은 단계 10 · 실제 HW 데이터 적재·운영 · AI 저장본·VZ `mission-history/images/robot/`와의 중복 정리(촬영본이 세 곳) — Phase 5/6 안건 · 다중 소스 병합은 HW #14 뒤 · 그전까지 HW 기본값(파이 로컬 적재)이 맞다 | — (단계 10에서 `tests/test_capture_upload.py`) |
