...
- [ ] **② 인프라 스택 compose 기동 + 헬스 확인 (Phase 0)** — 기존 설정 이관 파악 후 MK2 구조로
  재구성. 이 계획 문서 다음의 첫 착수.
- [ ] **③ 가짜 발행자로 얇은 파이프라인 관통 (Phase 1)** — 최소 발행자(C) → ingest → Kafka →
  저장 확인 → WS push 확인, 그다음 조병현 노드(A) 병행.
