...
from perception_framework.contracts.profile_loader import profile_from_dict, is_capability_active

# 도메인 프로파일은 simulator/scenarios/domain-river.json의 "profile" 필드에 있다.
profile = profile_from_dict(json.loads(open("simulator/scenarios/domain-river.json").read())["profile"])
