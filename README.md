...
- `pi/pi_base_setup.sh` — 말단 공통 베이스 셋업(식별자·시각동기·cgroup·venv·서비스). 멱등,
  `--role sensor|robot` 으로 센서노드/로봇 온보드를 갈라 프로비저닝
- `docs/SRS.md` — **소프트웨어 요구사항 명세서.** xlsx 30건을 검증 가능한 형태로 재기술하고
  타 파트 파생 제약·타당성 검토 결과·검증 매트릭스를 담은 기준 문서
- `docs/SDD.md` — **소프트웨어 설계 기술서.** 센서/로봇/액추에이터 3종 분할 구조,
  공통 코어(BaseNode), 주기 전환 3층, K3s 2층 배치, 장애 모드 대응
- `하드웨어_구현_착수순서_계획.md` — Phase 계획 + 결정사항 + 진행 기록
