...
| 구분 | ID | 수 | 요약 | 상태 |
|---|---|---:|---|---|
| 공통 규약 | BE-C | 7 | 공통 헤더·식별자 계층·frame_ref·좌표·계약축·도메인 프로파일·원천 종류 | 완료 1 · 미착수 6 |
...
| 저장·관측 | BE-S | 9 | 시계열 저장·OTel 파이프라인·관측 계층화·재난 보존·감사 MySQL·집약 표기·재난 SLA·임무 추적·미디어 저장 | 부분 3 · 미착수 6 |
...
**총 47건 · 완료 2 · 부분 5 · 미착수 40.** 완료는 BE-C-01(공통 헤더 범위)·BE-T-02(단일 머신
범위) 둘이며 Phase 1에서 pytest로 보장됐다. 부분 5건은 BE-T-01·BE-T-03·BE-S-01·BE-S-02·
BE-S-03이다. 각 행의 남은 범위와 gap은 [추적표](docs/be/requirement-traceability.md).
...
(Phase 2) ← 다음**. `store(...)` 인터페이스 뒤를 TSDB로 교체하고 감사·레지스트리 MySQL을
세운다.
...
| MySQL | 감사·레지스트리·임무 실행 기록 — 관계형 저장 축 (구간 4) | 기존 가동 |
