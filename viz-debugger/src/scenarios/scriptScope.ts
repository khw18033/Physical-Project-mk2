 ...
 *
 * **대본 목록에 묻는 것들** — 「이 축·이 탭·이 패널은 어느 편에서 살아나는가」.
 *
 * 규칙과 표는 옆의 `axes.ts` 하나에 있고, 여기는 그 규칙을 `SCRIPT_LIBRARY` 에 적용해
 * 답만 만든다. 갈라 둔 이유는 `verify:tab-scope` 가 표를 Node 에서 직접 읽어야 하는데
...
/** 이 탭이 살아나는 대본들 — 접힘 카드가 「그 대본으로 바꾸기」에 쓸 재료. */
export function scriptsUsingTab(tab: ScenarioTab): Array<{ missionId: string; title: string }> {
  return SCRIPT_LIBRARY
    .filter((entry) => entry.script !== null && tabsOfScript(entry.script).has(tab))
    .map((entry) => ({ missionId: entry.missionId, title: entry.script?.title ?? entry.missionId }));
}

/** 이 패널이 살아나는 대본들. 탭이 아니라 패널 단위 — 3편의 탭②는 살지만 구역 맵은 접힌다. */
