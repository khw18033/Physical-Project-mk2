...
export function TopBar({ onHome, onReplay }: { onHome(): void; onReplay(): void }) {
  ...
  return <header className="topbar">
    ...
    <nav><button onClick={() => command('mission_pause')}>■ 정지</button><button onClick={() => command('mission_resume')}>▶ 재시작</button><StopButton /><button onClick={onReplay}>◷ 임무 이력</button></nav>
  </header>;
}
