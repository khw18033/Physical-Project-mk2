 ...
 * 그때 함께 옮긴다.
 */
import { missionLabel, useMission } from '../data/scenario.ts';
...
export function TopBar({ onHome, onReplay }: { onHome(): void; onReplay(): void }) {
  ...
  return <header className="topbar">
    <button className="mission-home" onClick={onHome}><b>{current.missionId}</b><span>{missionLabel(current)}</span><small>목 데이터 · HCI 초안 · 클릭하면 마일스톤으로</small></button>
