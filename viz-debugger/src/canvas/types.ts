...
export type ViewNodeEntry = {
  ...
  inPalette?: boolean;
  /**
   * **이 임무의 팔레트에 두는가** (260915 — 자율주행 편). 없으면 모든 임무에 둔다.
   *
   * 문 찾기 시연의 노드(탐지 · pi7 로봇)와 자율주행 편의 노드(로봇 영상)는 서로의 임무에서 꺼낼 일이
   * 없다 — 꺼내면 남의 서버 값을 그 임무의 노드 옆에 그린다. `inPalette` 와 같이 **새로 꺼내는 길만**
   * 가른다. 이미 저장된 캔버스는 그대로 그린다.
   */
  showFor?(missionId: string): boolean;
};
