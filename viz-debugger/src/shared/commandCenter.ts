// 이식: web-dashboard/src/data/commands.ts @ 700ed91 — 앱의 단일 명령 출구로 승격
//
// **위치가 바뀐 것이 이 이식의 핵심이다.** 원본에서는 대시보드 탭들이 쓰는 데이터 계층의
// 한 파일이었지만, 통합 앱에서는 **앱 전체의 유일한 명령 출구**다. 그래서 tabs/data/ 가 아니라
// shared/ 에 있다 — 탭이 갖는 것이 아니라 앱이 갖는 것이다.
//
// 반대 방향(탭①의 25줄짜리 issueCommand 를 본체로 삼고 이걸 접기)을 고르면
// VZ-O-02(4단계 추적)와 VZ-O-03(감사 필드)이 통째로 사라진다. 탭①의 출구가 단순했던 것은
// 잘 설계돼서가 아니라 **받을 화면이 탭① 하나뿐이었기 때문**이다.
//
// 원본에서 고친 곳은 issue() 하나다 — 아래 주석 참조.
/**
 ...
 * 발행 옵션.
 *
 * `params` 와 `voice` 는 통합 때 더한 것이다. 주지 않으면 원본과 같은 요청이 나간다 —
 * 탭③(제어 패널)의 호출부는 한 줄도 바뀌지 않았고, 지금도 `{ target_pct }` 만 보낸다.
 */
export type IssueOptions = {
  ttlMs?: number;
  inputMode?: 'click' | 'voice' | 'api' | 'keyboard';
  /** 주지 않으면 `{ target_pct: spec.targetPct }`. 액추에이터가 아닌 명령이 쓴다. */
...
export class CommandTracker {
  ...
  async issue(
    ...
    const request: CommandRequest = {
      ...
      action: spec.action,
      // **원본에서 고친 곳 ①** — 원본은 `{ target_pct }` 만 보냈다. 액추에이터 명령만
      // 받으면 됐기 때문이다. 통합 앱에서는 임무 제어(정지·재시작·중단)와 발화 발행이
      // 같은 출구로 나가야 하는데 그것들은 target_pct 가 없다. 그래서 params 를 받되
      // **주지 않으면 원본과 완전히 같은 본문**이 되도록 뒀다.
      params: options.params ?? { target_pct: spec.targetPct },
      expires_at: expiresAt,
      // VZ-O-03 — 필드 이름은 auditFieldMap이 만든다. 여기서 이름을 쓰지 않는다.
      // **원본에서 고친 곳 ②** — 음성 감사 필드(REQ-1305)를 그대로 넘긴다. 이름을 붙이는
      // 일은 여전히 auditFieldMap 안에서만 일어난다.
