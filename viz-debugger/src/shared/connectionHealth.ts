...
export type HealthLine = {
  ...
  ok: boolean | null;
  /**
   * **이 줄이 어느 상대의 것인가** (260922). 로봇이 여럿이면 줄이 주소마다 셋씩이라,
   * 어느 주소의 결과인지 줄 스스로 말해야 한다.
   *
   * 글자다 — 주소는 번역하지 않는다(`reason` 과 같은 이유). 상대가 하나면 `undefined` 이고
   * 화면은 아무것도 안 적는다: 하나뿐일 때 주소를 반복하면 읽을 것만 는다.
   */
  scope?: string;
