 ...
 * 대시보드 데이터 계층이 실려 있으면 그 부하가 섞인 값이다.
 *
 * **260916 — 표시하는 자리가 셸에서 진입점으로 옮겨 갔다** (단독 빌드 정합 §3).
 *
 * 전에는 `shell/missionBridge.ts` 가 표시했고 「셸 = 통합」이 참이라 그걸로 맞았다.
 * 지금은 셸이 단독 빌드에도 들어가므로 그 자리에 두면 **단독이 스스로를 「통합」이라
 * 표시해 측정축 D 가 오염된다.**
 *
 * 그래서 `integrated.tsx` 가 표시한다. 이 표식의 뜻은 원래부터 「셸이 붙었는가」가 아니라
 * **「대시보드 데이터 계층이 실렸는가」**였고, 그것을 아는 곳은 그 계층을 주입하는
 * 진입점 하나뿐이다.
 */
