// **이 파일은 지금 아무도 쓰지 않는다** (260919 확인 — 영문화 4단계).
//
// `pipeline-contract` 를 import 하는 곳이 `src/` 에 하나도 없고, 이것을 쓰던 탭⑥은
// 2026-08-31 에 제거됐다(`verify:one-gateway` 주석). 그래서 아래 검증 메시지 20여 건은
// **화면에 뜨지 않는다.**
//
// 4단계에서 **일부러 안 옮겼다.** 안 보이는 글자를 사전에 넣으면 그 키가 영구히 남아
// 번역 검수 대상이 되고, 「이건 어디 뜨는 거냐」를 다음 사람이 매번 다시 묻는다.
// 파이프라인 편집기가 돌아오면 그때 이 파일과 함께 옮긴다 — 지울지 여부는 별개의 결정이다.
//
