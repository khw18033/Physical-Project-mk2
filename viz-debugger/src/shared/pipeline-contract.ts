// **화면에 닿지 않는 경로다 — 4단계에서 일부러 안 옮겼다** (260919).
//
// 처음에 「아무도 안 쓴다」고 적었는데 **틀렸다.** `gateway/` 셋이 이 파일을 import 한다.
//
//   gateway/pipeline-catalog.ts · gateway/pipeline-editor.ts · gateway/server.ts
//
// 코드는 돌고 `/pipelines/validate` · `/pipelines/test` 라우트도 살아 있다. 다만
// **그것을 소비하는 화면이 없다** — `server.ts` 가 그 자리에 이렇게 적어 뒀다:
//
//   「탭⑥(파이프라인 편집기)은 2026-08-31 제거 — 아래 /pipelines/* 는 소비하는 화면이 없다.」
//
// 그래서 아래 검증 메시지 20여 건은 **아무도 안 읽는 HTTP 응답 본문**으로 나간다.
// 안 보이는 글자를 사전에 넣으면 그 키가 영구히 남아 번역 검수 대상이 되고,
// 「이건 어디 뜨느냐」를 다음 사람이 매번 다시 묻는다. 기계가 옮겼던 9자리를 되돌렸다.
//
// 파이프라인 편집기가 돌아오면 그때 함께 옮긴다. 지울지 여부는 별개의 결정이다 —
// 게이트웨이가 쓰고 있으므로 지금 지우면 그쪽이 깨진다.
//
