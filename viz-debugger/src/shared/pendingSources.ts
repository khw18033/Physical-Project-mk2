...
export const PENDING_SOURCES: PendingSourceSpec[] = [
  ...
  {
    /**
     * 카메라 연결 상태 (260904 — 요구사항정의서 §7.10 「열림」).
     *
     * `VZ-D-07`(대상 상태 조회)에 넣으려는데 **`Hardware` 계약에 필드가 없다** —
     * 배터리·RSSI·지연·IP·펌웨어·관절 온도·하트비트는 있고 카메라만 없다. 하드웨어 시트에서
     * 대응 항목을 찾지 못했으므로 **상대를 지어내지 않고** 「상대 없음」으로 둔다.
     * `VZ-U-04` 가 「상대 없음」으로 삭제된 전례가 있다 — 만들기 전에 맞추는 것이 먼저다.
     */
    id: 'device-camera-link',
    title: '카메라 연결 상태 연결 예정',
    what: '대상에 달린 카메라가 붙어 있는가 · 스트림이 살아 있는가',
    from: [],
    missing: '하드웨어 계약에 해당 필드가 없다 — 다음 회의 안건 (요구사항정의서 §7.10)',
    ours: ['VZ-D-07'],
    plane: 'media',
  },
  {
