// 이식: web-dashboard/mock-gateway/server.ts @ 700ed91 — registry.json 경로 참조 · 임무 기록 채널 합류 · 포트 8790
/**
...
/** 임무 기록 재생 타이머 정리 함수. 종료 신호에서 부른다. */
let stopMissionTrace: (() => void) | null = null;
...
// **기동 시 계획을 올려 두지 않는다** (260918).
//
// 전에는 여기서 `plans.propose()` 로 데모 계획 하나를 승인 대기에 깔았다 — 「화면을 열자마자
// 승인 절차를 볼 수 있게」가 이유였다. 그런데 그 계획은 **진짜 계획 채널로 나가고 목 배지도
// 없어** 화면에서 진짜와 구별되지 않는다. `renderMode` 기본값을 `placeholder` 로 둔 이유가
// 바로 그것이다 — 진짜처럼 보이는 목이 시연에서 거짓말이 된다.
//
// 실제로 거짓말을 했다. 그 계획은 「503 구역 수위 상승 대응」인데 **503호와 수위는 아무
// 관계가 없다.** 문 찾기 시연 중 승인 탭에 그 계획이 떠 있었고, 260917 에 사람이 화면에서
// 찾았다(임무 다리가 끊겨 진짜 계획이 저장소에 안 들어오던 것과 겹쳐 더 오래 보였다).
//
// 원래 목적은 발화 → 대본 매칭 → `proposeScript()` 경로가 대신한다. 게다가 `PlanApproval`
// 은 `if (!acted) return null` 이라 **사람이 누르기 전엔 아예 안 그려서**, 이 계획은
// 「열자마자 보이게」 하지도 못한 채 나중에 대상 목록만 어지럽히고 있었다.
//
// 목 계획이 필요하면 **목·개발 모드의 devpanel** 에서 명시적으로 부른다(`plan-propose`).
// 그 모드는 붉은 배지가 유지되므로 지어낸 값이라는 사실이 화면에 남는다.
