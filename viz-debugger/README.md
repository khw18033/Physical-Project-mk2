...
마일스톤과 노드를 클릭해 다음 계층으로 이동하고, **머리줄의 이동 경로**(`마일스톤 › MS-… › T-…`)로 되돌아온다 — 맨 앞 칸이 마일스톤 목록이다 (2026-09-01). 마일스톤 카드 배정, 그래프 노드 자유 배치, DAG/트리 전환, `[이 마일스톤][임무 전체]` 범위 전환, 노드 더블클릭, 임무 이력 리플레이, 실패 파라미터 수정이 동작한다. 그래프는 **자리 폭을 재서 밴드로 접는다** — 「임무 전체」도 첫 화면에 다 들어온다. 계획 승인·거부(`VZ-U-07`)는 **마일스톤 목록 위 제안 카드**에 있고, 결정이 끝나면 `✓ 승인됨 · 시각 · plan_id · [근거 ▾]` 한 줄로 접힌다. 정적 전달본과 논의 항목은 [HANDOFF.md](./HANDOFF.md)에 정리했다.
...
화면의 설명 문단은 우상단 `?`(현재 탭의 설명서)로 들어갔다.

**시나리오 모드에서는 화면이 대본을 따라간다** (2026-09-01). 세 층이다 —
① 대본이 안 쓰는 **탭**은 탭 바에서 흐려지고 `· 이 대본엔 없음` 이 붙는다(막지는 않는다).
② 대본이 안 쓰는 **패널**은 `PanelGate`/`TabGate` 가 제목·버튼 줄·표까지 통째로 접고 한 장짜리
안내로 바꾼다 — 1·2편은 탭③, 3편은 탭⑤가 접힌다. ③ 남은 **자리**는 대본 값이거나
「이 대본에는 해당 없음」이다. 축→탭·패널→축 표는 `src/scenarios/axes.ts` 하나에 있고
...
| `verify:layout` | 드래그로 노드와 연결선이 함께 움직이는가. **계산된 배치가 주어진 폭 안에 들어오는가** *(260901 확장)* — 대본 3편 × DAG/트리 × 임무 전체·마일스톤별 × 폭 1120/1280/1440/1920px 에서 노드가 화면 밖으로 나가거나 겹치지 않아야 한다. 접지 않은 배치가 3,290px 로 걸리는 음성 대조군 포함 |
