...
//
// ## 규칙 (260921 — 검사 줄바꿈 지시서 §1)
//
// > **대조군은 LF 로 정규화한 원본에서 만든다.** 즉 `readSource()` 로 읽는다.
//
// 여러 줄 자리표를 쓰는 검사를 새로 쓸 때 이 한 줄만 지키면 된다. 지켰는지는 사람이
// 기억하지 않는다 — **`verify:crlf-safe` 가 소스를 보고 잡는다.** 자리표에 `\n` 이 있는데
// 맨 `readFileSync` 로 읽으면 실패한다.
//
// `\r?\n` 정규식으로 **찾는 쪽을 고치지 마라.** 대조군이 늘 때마다 사람이 기억해야 하는
// 규칙이 되고, 한 번 잊으면 조용히 통과한다. 입구에서 한 번 하면 잊을 자리가 없다.
//
// ## 왜 필요했나
//
