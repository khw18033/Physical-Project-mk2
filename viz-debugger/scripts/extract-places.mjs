...
/**
 * 415호는 이 씬에 없다 — 4층이고 실측 대상이 아니다. 그런데 옛 편(MSN-260826-01)의
 * 발화가 「415호에서 503호로」이므로 자리가 없으면 그 대본의 장소를 못 푼다.
 * **자리만 두고 인접은 비운다** — 「연결 예정」과 같은 원칙이다(지시서 §1).
 */
places.push({
  place_id: 'room-415', label: '415호', aliases: ['415', '415호'],
  kind: 'room', floor: 4, adjacent: [], zone_id: null,
});
