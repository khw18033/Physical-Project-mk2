 ...
 *
 * ## 영어 판에서는 이 표지가 하나도 안 걸린다 (260919 — 영문화 5단계 뒤)
 *
 * `run-baseline.mjs --lang en` 은 이 파일을 부를 때 **언어를 안 넘긴다.** 그래서 영어 발화에
 * 대해 `asked.branch` · `asked.loop` 가 **언제나 `false`** 다.
 *
 * **지금은 그래도 맞는 값이 나온다.** 이 축이 재는 것은 정확도가 아니라 **절제**이고
 * (`axisPlanShape` 의 `note`), 영어 정답셋 발화 어디에도 루프·분기를 요구하는 말이 없다 —
 * 한국어 15건과 같은 성질이다. 요구가 없으니 `asked` 가 `false` 인 것이 옳다.
 *
 * **루프나 분기를 요구하는 영어 발화를 하나라도 더하는 날 틀어진다.** 그때는 맞게 만든
 * 분기가 `invented` 로 세어지고, 점수가 나온 뒤에야 이상해 보인다. 더하기 전에 영어 표지를
 * 옆에 두고 `--lang` 을 여기까지 흘려라 — 3단계가 `match_en` 으로 한 것과 같은 모양이다.
 * `src/generate/planShape.ts` 의 같은 주석이 화면 쪽 사정을 적고 있다.
 */
