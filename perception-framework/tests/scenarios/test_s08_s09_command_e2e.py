"""implements: AI-B-03, AI-C-06, AI-C-14, AI-O-01
"""
import socket
import time
import uuid
...
from perception_framework.simulation.terminals import CommandOutcome, VirtualRobotTerminal
...
# AI-C-07: the test encodes with the same policy-selected serializer the
# terminal decodes with — it never assumes a concrete wire format.
_CONTROL_SERIALIZER = SerializationPolicy.default().serializer_for("control")


def command_payload(command: str, **params) -> bytes:
    return _CONTROL_SERIALIZER.encode(
...
def test_s8_command_travels_server_to_terminal_and_the_result_comes_back():
    ...
    # 결과가 서버까지 되돌아왔다.
    returned = [_CONTROL_SERIALIZER.decode(p)["outcome"] for p in results]
...
def test_s9_command_is_rejected_on_a_precondition_violation_but_delivery_succeeded():
    ...
    assert any(_CONTROL_SERIALIZER.decode(p)["reason"] == "insufficient_battery" for p in results)
...
def test_s8_unknown_command_fails_with_a_reason_rather_than_silently():
    ...
    robot._on_command(b"\xff\xfe-undecodable")
...
def test_s8_command_e2e_over_real_kafka_and_mqtt():
    ...
    try:
        ...
        edge_server_side.publish(f"srv.robot.result.{suffix}", _CONTROL_SERIALIZER.encode({"outcome": "WARMUP"}))
        time.sleep(1.0)
        ...
        while time.time() < deadline and not any(
            _CONTROL_SERIALIZER.decode(p).get("outcome") == "SUCCESS" for p in results
        ):
            time.sleep(0.1)
    finally:
        device.close()
    ...
    assert any(_CONTROL_SERIALIZER.decode(p).get("outcome") == "SUCCESS" for p in results)
