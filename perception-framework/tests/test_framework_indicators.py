...
REPORT_PATH = Path(__file__).resolve().parents[1] / "reports" / "framework-indicators.json"
# was `profiles/*.json` - domain profiles now live embedded in simulator scenarios (item 10).
DOMAIN_SCENARIO_DIR = Path(__file__).resolve().parents[1] / "simulator" / "scenarios"
DOMAIN_SCENARIO_FILES = sorted(DOMAIN_SCENARIO_DIR.glob("domain-*.json"))
...
def measure_extensibility() -> dict:
    ...
    for path in DOMAIN_SCENARIO_FILES:
        tokens[f"domain::{path.stem}"] = _load_domain_profile(path).domain_id
...
def measure_domain_generality() -> dict:
    ...
    for path in DOMAIN_SCENARIO_FILES:
        registry = CapabilityRegistry()
        profile = _load_domain_profile(path)
