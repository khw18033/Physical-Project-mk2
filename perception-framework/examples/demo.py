...
from perception_framework.reference.local_safety import LocalSafetyJudge
...
# ---------------------------------------------------------------------------
section("2. AI-S-06 이종 근거의 점진적 객체 레코드")
# ---------------------------------------------------------------------------
...
arrivals = [
    Evidence("e1", "frame-0", "det_a", 0.195, 0.195, "region", 0.82, (0, 0, 10, 10), "cup"),
    Evidence("e2", "frame-0", "segmenter", 0.448, 0.448, "mask", 0.71, (1, 1, 9, 9)),
    Evidence("e3", "frame-0", "det_b", 1.302, 1.302, "region", 0.64, (0, 0, 11, 10), "bowl"),
]
for evidence in arrivals:
    revision = builder.ingest("obj-1", evidence)
    if revision:
        print(f"  {evidence.available_at:.3f}s {evidence.source_group:10s}"
              f" -> rev{revision.revision} {revision.semantic_class} ({revision.geometry_kind})")
print("  (마지막 det_b가 다른 클래스를 주장하지만 지지 생산자 수가 같아 기존 클래스를 유지)")

lost = builder.ingest(
    "obj-1",
    Evidence("e4", "frame-0", "det_b", 1.9, 1.9, "region", 0.64, (0, 0, 11, 10), "bowl"),
    available_groups={"det_a", "segmenter"},
)
print(f"  det_b 소실 후 재도착 -> 갱신 {'없음' if lost is None else '발생'}"
      f" / lifecycle={builder.records['obj-1'].lifecycle.value}")
print("  (사라진 생산자의 부재는 반대 근거가 아니므로 상태가 흔들리지 않음)")
...
from perception_framework.providers.compute import (  # noqa: E402
    TAG_CPU,
    TAG_OPENCL,
...
from perception_framework.contracts.profile_loader import is_capability_active, load_profile  # noqa: E402
...
for name in ("robot", "facility", "river"):
    ...
    print(
        f"{profile.domain_id:26s} 환경맵={is_capability_active(profile, 'perception.environment_map')}"
