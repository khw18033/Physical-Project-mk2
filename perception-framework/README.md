...
구조 설명은 [docs/ai/architecture.md](../docs/ai/architecture.md) 참고. 패키지 자체의
...
perception_framework/
  ...
  execution/      # 실행제어/lifecycle/conformance/물리명령 supervisor+MQTT gateway (AI-C-20)
  ...
  simulation/     # 가상 말단·replay 소스·백엔드/디지털트윈 mock — 하드웨어 없이 시나리오 검증
  ondevice/       # 환경·보정 설정 적용 (AI-N-02, AI-N-03)
simulator/        # simulation.py(실행) + scenarios/*.json — 하드웨어 없이 프레임워크 성질 확인,
                  # 같은 JSON이 tests/test_scenarios.py의 자동 검증 입력도 겸함
```
...
```bash
pip install -e ".[dev]"                    # 선택 extra: mqtt / kafka / otel / sim(시뮬레이터용 pyjevsim)
pytest -q                                  # 전체 테스트
# 시스템 pytest 플러그인 충돌 시:
PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python3 -m pytest -q
PYTHONPATH=. python3 simulator/simulation.py     # 하드웨어 없이 프레임워크 성질 확인하는 시뮬레이터
pytest -q tests/test_scenarios.py                # 같은 시나리오(simulator/scenarios/*.json)를 자동 검증으로
pytest -q tests/test_integration_functional_check.py  # 모듈 단위 동작 통합 확인(실패 항목별 출력)
```

`tests/test_framework_indicators.py`는 매 실행마다 지표를 새로 측정해
...
실제 Mosquitto·Kafka·OTel Collector와 기존 K3s를 연결하는 재현 절차는
[`deploy/integration/README.md`](deploy/integration/README.md)에 있다. 2026-09-02 해당
구성과 POCL OpenCL을 사용한 전체 결과는 `531 passed, 0 skipped`였다. 이 로컬 stack은
loopback plaintext 검증용이며 운영 인증·ACL 구성을 대신하지 않는다.
