...
def onnx_execution_providers() -> tuple[str, ...]:
    """Return execution providers visible to ONNX Runtime on this node."""
    try:
...
def onnx_provider_available(provider_name: str) -> bool:
    return provider_name in onnx_execution_providers()


def onnx_accelerator_available() -> bool:
    """Whether ONNX Runtime exposes any non-CPU execution provider.

    The concrete provider names stay data values. Selection code uses the
    generic `compute.gpu` tag, so a node can swap accelerator runtimes without
    changing upper-layer AI logic.
    """
    providers = onnx_execution_providers()
    return any(name != "CPUExecutionProvider" for name in providers)
...
class OnnxRuntimeProvider:
    """Generic ONNX Runtime-backed `AIRuntimeProvider`.

    This provider is intentionally model-agnostic: capability mapping is
    supplied at registration time, while the selected execution provider is a
    runtime option. A Raspberry Pi CPU model and a control-node accelerator
    model therefore satisfy the same `AIRuntimeProvider` contract.
    """
    ...
    def __init__(
        ...
        model_path: str | Path,
        *,
        capability_kinds: Sequence[str],
        provider_id: str,
        execution_providers: Sequence[str] = ("CPUExecutionProvider",),
    ) -> None:
        self.model_path = Path(model_path)
        self.capability_kinds = tuple(capability_kinds)
        self.provider_id = provider_id
        self.execution_providers = tuple(execution_providers)
        self._session: Any | None = None

    def is_available(self) -> bool:
        if not self.model_path.exists():
            return False
        available = set(onnx_execution_providers())
        return all(provider in available for provider in self.execution_providers)

    def infer(self, capability_kind: str, inputs: Any) -> Any:
        if capability_kind not in self.capability_kinds:
            raise ValueError(f"unsupported capability_kind: {capability_kind}")
        if not self.is_available():
            raise RuntimeError("ONNX Runtime provider is not available")
